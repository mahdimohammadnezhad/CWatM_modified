...
ForcingStart = datetime.datetime.strptime(parser.get(
    'DEFAULT', 'ForcingStart'), "%d/%m/%Y")  # Start of forcing
...
SubCatchmentPath = os.path.join(
    rootbasin, parser.get('Path', 'SubCatchmentPath'))
...
# using default run as first run
firstrun = parser.getboolean('Option', 'firstrun')
if firstrun:
...
runmodel = os.path.splitext(os.path.join(
    rootbasin, modeltemplate, RunModel_template))[0]
...
streamflow_data = pandas.read_csv(
    Qtss_csv, sep=",", parse_dates=True, index_col=0)
...
if firstrun:
    ...
    for ii in range(0, len(ParamRanges - 1)):
        ...
        if delta == 0:
            ...
            para_first2.append(
                (para_first[ii] - float(ParamRanges.iloc[ii, 0])) / delta)
...
def RunModel(Individual):
    ...
    for ii in range(0, len(ParamRanges-1)):
        Parameters[ii] = Individual[ii]*(float(ParamRanges.iloc[ii, 1])-float(
            ParamRanges.iloc[ii, 0]))+float(ParamRanges.iloc[ii, 0])
    ...
    for ii in range(0, len(ParamRanges)-1):
        template_xml_new = template_xml_new.replace(
            "%"+ParamRanges.index[ii], str(Parameters[ii]))
    ...
    if runmodel:
        ...
        f = open(os.path.join(
            directory_run, ModelSettings_template[:-4] + '-Run' + run_rand_id + '.ini'), "w")
        f.write(template_xml_new)
        f.close()

        template_bat_new = template_bat
        template_bat_new = template_bat_new.replace(
            '%run', ModelSettings_template[:-4]+'-Run'+run_rand_id+'.ini')
        runfile = os.path.join(
            directory_run, RunModel_template[:-4]+run_rand_id)
        if platform == "win32":
            runfile = runfile + ".bat"
            if use_multiprocessing == 0:
                print("multiprocess off")
                template_bat_new = template_bat_new.split(
                )[0] + " " + template_bat_new.split()[1] + " " + template_bat_new.split()[2] + " -l\npause"
        else:
            runfile = runfile + ".sh"
            if use_multiprocessing == 0:
                template_bat_new = template_bat_new.split(
        ...
        p = Popen(runfile, shell=True, stdout=PIPE,
                  stderr=PIPE, bufsize=16*1024*1024)
    ...
    if os.path.isfile(Qsim_tss) == False:
        # os.system(f"{directory_run}/runpy{run_rand_id}.bat")
        # result = subprocess.run(
        #     [f"{directory_run}/runpy{run_rand_id}.bat"],
        #     shell=True,
        #     check=True,         # optional: raises an exception on non-zero exit code
        #     capture_output=True,
        #     text=True
        # )
        # print("Batch finished with code", result.returncode)
    ...
    simulated_streamflow = pandas.read_csv(
        Qsim_tss, sep=r"\s+", index_col=0, skiprows=4, header=None, skipinitialspace=True)
...
toolbox.register("Individual", tools.initRepeat,
                 creator.Individual, toolbox.attr_float, len(ParamRanges))
...
if __name__ == "__main__":
    ...
    if startlater == False:
        ...
        for ii in range(1):
            effmax[0, ii] = np.amax([halloffame[x].fitness.values[ii]
                                    for x in range(len(halloffame))])
            effmin[0, ii] = np.amin([halloffame[x].fitness.values[ii]
                                    for x in range(len(halloffame))])
            effavg[0, ii] = np.average(
                [halloffame[x].fitness.values[ii] for x in range(len(halloffame))])
            effstd[0, ii] = np.std([halloffame[x].fitness.values[ii]
                                   for x in range(len(halloffame))])
        gen = 0
        print(">> gen: "+str(gen)+", effmax_KGE: " +
              "{0:.3f}".format(effmax[gen, 0]))
        gen = 1
    ...
    while not any(conditions.values()):
        ...
        ), offspring=offspring, halloffame=halloffame)
        with open(checkpoint, "wb") as cp_file:
            pickle.dump(cp, cp_file)
        cp_file.close()
        startlater = False
        ...
        # from the Pareto optimal population
        for ii in range(1):
            effmax[gen, ii] = np.amax(
                [halloffame[x].fitness.values[ii] for x in range(len(halloffame))])
            effmin[gen, ii] = np.amin(
                [halloffame[x].fitness.values[ii] for x in range(len(halloffame))])
            effavg[gen, ii] = np.average(
                [halloffame[x].fitness.values[ii] for x in range(len(halloffame))])
            effstd[gen, ii] = np.std(
                [halloffame[x].fitness.values[ii] for x in range(len(halloffame))])
        print(">> gen: "+str(gen)+", effmax_KGE: " +
              "{0:.3f}".format(effmax[gen, 0]))
    ...
    for kk in range(len(halloffame)):
        for ii in range(len(ParamRanges)):
            paramvals[kk][ii] = halloffame[kk][ii]*(float(ParamRanges.iloc[ii, 1])-float(
                ParamRanges.iloc[ii, 0]))+float(ParamRanges.iloc[ii, 0])
    ...
    pareto_front = pandas.DataFrame(
        {'effover': effover[ind], 'R': front[ind, 0]})
    for ii in range(len(ParamRanges)):
        pareto_front["param_"+str(ii).zfill(2)+"_" +
                     ParamRanges.index[ii]] = paramvals[ind, ii]
    ...
    if bestrun:
        ...
        for ii in range(0, len(ParamRanges)):
            template_xml_new = template_xml_new.replace(
                "%"+ParamRanges.index[ii], str(Parameters[ii]))
        template_xml_new = template_xml_new.replace(
            '%run_rand_id', directory_run)

        os.mkdir(directory_run)

        # template_xml_new = template_xml_new.replace('%InitModel',"1")
        f = open(os.path.join(directory_run,
                 ModelSettings_template[:-4]+'-Run'+run_rand_id+'.ini'), "w")
        f.write(template_xml_new)
        f.close()
        template_bat_new = template_bat
        template_bat_new = template_bat_new.replace(
        ...
        simulated_streamflow = pandas.read_table(
            Qsim_tss, sep=r"\s+", index_col=0, skiprows=4, header=None, skipinitialspace=True)
        simulated_streamflow[1][simulated_streamflow[1] == 1e31] = np.nan
        ...
        Qsim = pandas.DataFrame(data=Qsim, index=pandas.date_range(
            ForcingStart, periods=len(Qsim), freq=frequen))
        Qsim.to_csv(os.path.join(path_subcatch,
                    "streamflow_simulated_best.csv"), ',', header="")
        try:
            os.remove(os.path.join(path_subcatch, "out",
                      'streamflow_simulated_best.tss'))
        except:
            pass
